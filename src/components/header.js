...
function Header({ altLangs, settings }) {
  ...
  return (
    <>
      <header
        className={`fixed bg-white transition-all duration-300 left-0 z-[999] top-0 w-full px-1 md:py-1 pb-2 md:px-4 font-secondary  h-5 flex items-center md:pt-1 pt-3`}
      >
            ...
            {settings.data?.header[0]?.link?.map((item, i) => {
              return (
                <PrismicLink
                  key={item.key}
                  field={item}
                  className={`text-sm header_link tracking-[0.08em]  ${router.asPath.includes(item.text.toLowerCase()) ? "active_link" : ""} uppercase text-blue  font-secondary leading-none relative group`}
                >
                  {item.text}
                </PrismicLink>
              );
